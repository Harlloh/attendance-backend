...
export const openSession = async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to open session." });
  }
};
...
export const closeSession = async (req, res) => {
  ...
  try {
    ...
    if (!sessionExist) {
      return res
        .status(400)
        .json({ success: false, message: "Session does not exist." });
    }
    if (!sessionExist.isOpen) {
      return res.status(400).json({
        success: false,
        message: "Session is closed already",
    ...
    res
      .status(200)
      .json({ success: true, message: "session closed successfully", session });
  } catch (error) {
    console.error(error);
    res
      .status(500)
...
export const manualAssignment = async (req, res) => {
  ...
  if (!sessionId || !stateCode || !name) {
    return res.status(400).json({
      success: false,
      message: "sessionId, stateCode and name are required.",
    });
  }
  const adminId = req.admin.id;
  try {
    ...
    if (!confirmSession) {
      return res
        .status(400)
        .json({ success: false, message: "Session cannot be found" });
    }
    const existingByStateCode = await redis.get(
    ...
    if (existingByStateCode) {
      ...
      return res.status(409).json({
        success: false,
        message: "Corps member already has a number for this session.",
        queueNumber: existingByStateCode.queueNumber,
      });
    }

    // 2. Fallback duplicate check in Postgres (cache miss safety net)
    const existingRecord = await prisma.attendanceRecord.findUnique({
      where: { sessionId_stateCode: { sessionId, stateCode } },
    });
    if (existingRecord) {
      return res.status(409).json({
        success: false,
    ...
    // 3. Use the SAME Redis counter as self-check-in
    ...
    await Promise.all([
      ...
      // No device key — admin assignments have no fingerprint
    ]);

    // 5. Write to Postgres synchronously for admin (admin can wait, accuracy matters more)
    ...
    res
      .status(500)
      .json({ success: false, message: "Failed to assign number." });
  }
};

// export const handleNumberAssignment = async (
//   addedByAdmin,
//   sessionId,
//   stateCode,
//   name,
...
export const getAttendanceList = async (req, res) => {
  ...
  if (!pageSize || !pageIndex) {
    return res.status(400).json({
      success: false,
      message: "The pagesize and page index is required",
    });
  }
...
export const validateUser = async (req, res) => {
  ...
  if (!sessionId || !stateCode || !queueNumber || !checkInSlug) {
    return res.status(400).json({
      success: false,
      message:
        "sessionId, stateCode, queueNumber and checkInSlug are required.",
    });
  }
  ...
  try {
    ...
    if (!cachedSession) {
      return res.status(400).json({
        success: false,
        message:
          "The session for this qr code has expired. from invalid session in cache",
